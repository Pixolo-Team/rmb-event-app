...
- **Check-in card & countdown (Home, F3.7)** — the inline check-in prompt (icon + copy + a single design-system primary button that runs location → in-app venue-QR scanner), the compact "Checked in ✓ · time" strip, and the pre-event countdown block (eyebrow + event name/date + a 4-part days/hrs/min/sec timer shown only within 3 days of start). No bespoke/inline-styled buttons — all use the standard button set.
- **Venue attendance QR card (Admin, F3.7)** — the printable venue QR with Download PNG (primary) + Regenerate (secondary) actions and a token snippet.
