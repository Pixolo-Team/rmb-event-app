...
const PREVIEW: LeaderboardResponse = { top: [
...
], me: { id: "me", rank: 4, name: "Radha Sharma", businessName: "Radha Textiles", photoUrl: null, metCount: 8 }, totalAttendees: 42, updatedAt: new Date().toISOString() };
...
export default function LeaderboardPage() {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      leaderboardCache.set(result); setData(result); setOffline(false); setError(false);
    } catch {
      const cached = leaderboardCache.get();
      if (cached) { setData(cached); setOffline(true); } else setError(true);
