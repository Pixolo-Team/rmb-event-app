...
import type { FeedAttendee, FeedPhotoData } from "../lib/feedTypes";

const PREVIEW_ATTENDEE: FeedAttendee = { id: "preview-me", name: "Radha Sharma", businessName: "Sharma Trading Co." };
...
export default function FeedPage() {
  const [attendee, setAttendee] = useState<FeedAttendee | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/attendees/me", { credentials: "include" }).then(async (response) => {
      ...
      setAttendee(await response.json() as FeedAttendee);
    }).catch(() => setError(true));
  }, []);
